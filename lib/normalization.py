...
resize = lambda x, image_size, scale_factor: F.interpolate(x.unsqueeze(0).cuda(),
                                                           size=(int(np.floor(x.shape[1] / (np.max(x.shape[
                                                                                                   1:]) / image_size) * scale_factor) / scale_factor),
                                                                 int(np.floor(x.shape[2] / (np.max(x.shape[
                                                                                                   1:]) / image_size) * scale_factor) / scale_factor)),
                                                           mode='bilinear', align_corners=False)

padim = lambda x, h_max: torch.cat((x, x.view(-1)[0].clone().expand(1, 3, h_max - x.shape[2], x.shape[3]) / 1e20),
                                   dim=2) if x.shape[2] < h_max else x